      ...
      if (autoInjectTraits && currentProject.characters && currentProject.characters.length > 0) {
        ...
        currentProject.characters.forEach(char => {
          ...
          if (regex.test(editorPrompt)) {
            ...
            const tokens = buildCharacterTokens(char.details);
      ...
      {showFirebaseModal && (
        <div className="modal-overlay">
          <div className="modal-content glass-panel" style={{ maxWidth: '550px' }}>
            <div className="modal-header">
              <h2>Conectar Firebase (Sincronización en la Nube)</h2>
              <button onClick={() => setShowFirebaseModal(false)} className="modal-close">✕</button>
            </div>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '16px' }}>
              Configura tus credenciales de Firebase para habilitar Google Authentication real y guardar tus mangas en la nube (Firestore) para que otros usuarios puedan verlos en tiempo real.
            </p>
            <form onSubmit={handleSaveFirebaseConfig}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                ...
                <div className="form-group">
                  ...
                  <input 
                    ...
                    onChange={(e) => setFirebaseConfigInput({...firebaseConfigInput, authDomain: e.target.value})}
                  />
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input 
                    ...
                    onChange={(e) => setFirebaseConfigInput({...firebaseConfigInput, storageBucket: e.target.value})}
                  />
                </div>
              </div>
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: '20px' }}>
                ...
                <button type="button" onClick={() => setShowFirebaseModal(false)} className="btn-secondary">Cerrar</button>
